...
namespace CsvHelper.Configuration
{
	...
	public class Configuration : IReaderConfiguration, IWriterConfiguration
	{
		...
		/// </summary>
		public virtual Action<bool, string[], int, IReaderContext> HeaderValidatedCallback { get; set; } = ( isValid, headerNames, headerNameIndex, context ) =>
		{
			if( isValid )
			{
				return;
		...
		/// </summary>
		public virtual Action<string[], int, IReaderContext> MissingFieldFoundCallback { get; set; } = ( headerNames, index, context ) =>
		{
		...
		/// Arguments: context
		/// </summary>
		public virtual Action<IReaderContext> BadDataFoundCallback { get; set; } = context =>
		{
			throw new BadDataException( context, $"Field: '{context.ParserContext.Field}'. You can ignore bad data by setting {nameof( BadDataFoundCallback )} to null." );
		};
